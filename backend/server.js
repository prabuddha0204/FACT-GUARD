...
if (process.env.FIRECRAWL_API_KEY) {
  ...
  console.log("Firecrawl agent enabled");
} else {
  console.log("Firecrawl API key missing — using fallback browsing");
}
...
function detectScamSignals(text) {
  ...
  if (/immediately|blocked|suspended|expire|urgent|last chance/i.test(text))
    signals.push("Urgency Language Detected");
  if (/sbi|hdfc|icici|npci|uidai|rbi|axis|kotak|paytm/i.test(text))
    signals.push("Bank / Authority Impersonation Detected");
  if (/send|transfer|pay\s*₹|pay\s*rs|deposit/i.test(text))
    signals.push("Money Transfer Request Detected");
  if (/bit\.ly|tinyurl|cutt\.ly|shorturl/i.test(text))
    signals.push("Suspicious Short URL Detected");
  if (/won|winner|prize|lottery|congratulations|selected|free/i.test(text))
    signals.push("Fake Prize / Offer Detected");
  if (/password|pin\b|cvv|card.?number|account.?number/i.test(text))
    signals.push("Sensitive Data Request Detected");
  if (/kyc|know your customer|verify your account|update your details/i.test(text))
    signals.push("Fake KYC Update Request Detected");
  if (/income.?tax|it.?department|tax.?refund|tds/i.test(text))
    signals.push("Fake Government / Tax Authority Detected");
  return signals;
}
  ...
  try {
    ...
    const aiResponse = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a cybersecurity AI specialized in detecting phishing websites.

Analyze webpage content and determine if it is SAFE, SUSPICIOUS, or PHISHING.

Look for:
- fake login forms requesting credentials
- impersonation of banks, payment apps, or government websites
- urgent security warnings forcing user action
...
breakdown must have 2-3 points.`,
        ...
        {
          role: "user",
          content: `Analyze this webpage for phishing risk.

Submitted URL: ${url}
Resolved URL: ${expandedUrl}
...
Format:
{
  ...
  "breakdown": [{ "point": "short label", "detail": "one sentence" }],
